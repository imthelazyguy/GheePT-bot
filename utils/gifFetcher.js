...
async function getGif(searchTerm, nsfw = false) {
    if (!TENOR_API_KEY) {
        console.error("TENOR_API_KEY is not configured. Please add it to your environment variables.");
        return FALLBACK_GIF;
    }
    const safeSearch = nsfw ? 'off' : 'medium';
    const url = `https://tenor.googleapis.com/v2/search?q=${encodeURIComponent(searchTerm)}&key=${TENOR_API_KEY}&limit=25&media_filter=minimal&contentfilter=${safeSearch}`;
